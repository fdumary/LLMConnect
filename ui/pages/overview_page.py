...
OVERVIEW_HTML_PATH = os.path.join(os.path.dirname(__file__), "overview.html")
...
class OverviewBridge(QObject):
    def __init__(
        ...
        self.on_add_browser_model = on_add_browser_model
        self.on_add_api_key = on_add_api_key
...
class OverviewPage(QWidget):
    def __init__(self, on_navigate, on_add_browser_model, on_add_api_key):
        super().__init__()
        self._loaded = False
        self._payload = None
        ...
        self.bridge = OverviewBridge(
            self, on_navigate, on_add_browser_model, on_add_api_key
        )
    ...
    def set_payload(self, payload):
        ...
        if not self._loaded:
            ...
            with open(OVERVIEW_HTML_PATH, "r", encoding="utf-8") as handle:
                template = handle.read()
            html = template.replace("__APP_DATA__", json_data)
            base = QUrl.fromLocalFile(os.path.dirname(OVERVIEW_HTML_PATH) + os.sep)
