...
PROJECTS_HTML_PATH = os.path.join(os.path.dirname(__file__), "projects.html")
...
class ProjectsBridge(QObject):
    def __init__(self, on_navigate, on_add_project):
        super().__init__()
...
class ProjectsPage(QWidget):
    def __init__(self, on_navigate, on_add_project):
        ...
        self.bridge = ProjectsBridge(on_navigate, on_add_project)
    ...
    def set_payload(self, payload):
        ...
        if not self._loaded:
            ...
            with open(PROJECTS_HTML_PATH, "r", encoding="utf-8") as handle:
                template = handle.read()
            html = template.replace("__APP_DATA__", json_data)
            base = QUrl.fromLocalFile(os.path.dirname(PROJECTS_HTML_PATH) + os.sep)
