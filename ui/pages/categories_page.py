...
CATEGORIES_HTML_PATH = os.path.join(os.path.dirname(__file__), "categories.html")
...
class CategoriesBridge(QObject):
    def __init__(self, on_navigate, on_add_category):
        super().__init__()
...
class CategoriesPage(QWidget):
    def __init__(self, on_navigate, on_add_category):
        ...
        self.bridge = CategoriesBridge(on_navigate, on_add_category)
    ...
    def set_payload(self, payload):
        ...
        if not self._loaded:
            ...
            with open(CATEGORIES_HTML_PATH, "r", encoding="utf-8") as handle:
                template = handle.read()
            html = template.replace("__APP_DATA__", json_data)
            base = QUrl.fromLocalFile(os.path.dirname(CATEGORIES_HTML_PATH) + os.sep)
